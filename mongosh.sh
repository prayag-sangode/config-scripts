...
sudo cp mongosh-2.1.4-linux-x64/bin/mongosh /usr/local/bin/

# Move the mongosh_crypt_v1.so library to /usr/local/lib/
sudo cp mongosh-2.1.4-linux-x64/bin/mongosh_crypt_v1.so /usr/local/lib/

# Create a symbolic link to the mongosh binary in /usr/local/bin/
sudo ln -s /usr/local/bin/mongosh-2.1.4-linux-x64/bin/* /usr/local/bin/
