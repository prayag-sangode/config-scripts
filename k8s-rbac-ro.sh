...
roleRef:
  ...
  kind: ClusterRole
  name: $username-read-only" | kubectl apply -f -
...
kubeconfig=kubeconfig-${namespace}-${username}-read-only-$(date '+%F-%H%M%S')
...
echo "$username's kubeconfig with read-only access was created into `pwd`/$kubeconfig"
